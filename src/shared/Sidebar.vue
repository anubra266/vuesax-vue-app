<template>
  <vs-sidebar v-model="active" :open.sync="activeSidebar">
    <template #logo> <SiteLogo /><b>ue</b> - Boilerplate</template>
...
<script>
import SiteLogo from "@/shared/logo";
export default {
  props: ["mobile"],
  components: { SiteLogo },
  computed: {
    active: {
      ...
      set(navbarActive) {
        return this.$store.dispatch("setActive", navbarActive);
      },
    },
    activeSidebar: {
      get() {
        // const status = this.$store.state.activeSidebar;
        const status = this.$store.getters.sidebarState;
        return this.mobile ? status : true;
      },
      set(sidebarState) {
